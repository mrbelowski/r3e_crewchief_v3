...
Changelog
---------
Version 3.6.2: Reset game state when restarting a race (will still fail if you restart during the countdown phase); Added "who's in front / behind on track" response - gives driver name & position of the guy in front / behind regardless of his race position (if he's moving and not in the pits); Fixed some issues in the spotter handling re-ordered participants array; Make opponent speed calculation handle noise data better; Added options to set the minimum voice recognition confidence for regular requests ("what's my position?" and stuff - defaults to 50%), and a separate confidence level for driver-name requests ("where's Bob?" - defaults to 40%); Fixed app playing messages when monitoring other cars; Fixed app not running in free practice mode. No new sound pack for this version.
